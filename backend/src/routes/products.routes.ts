...
router.post('/', async (req: ClientContextRequest, res: Response) => {
  try {
    if (req.clientId === undefined || req.clientId === null) {
      return res.status(400).json({ success: false, error: 'Client must be selected to create a product.' });
    }
...
router.put('/:id', async (req: ClientContextRequest, res: Response) => {
  try {
    ...
    if (req.clientId === undefined || req.clientId === null) {
      return res.status(400).json({ success: false, error: 'Client must be selected to update a product.' });
    }
...
router.delete('/:id', async (req: ClientContextRequest, res: Response) => {
  try {
    ...
    if (req.clientId === undefined || req.clientId === null) {
      return res.status(400).json({ success: false, error: 'Client must be selected to delete a product.' });
    }
